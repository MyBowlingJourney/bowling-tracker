...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    // The bowler's own notes, transcribed -- not applied.
    //
    // Told to use a note like "4" under the tenth for the fill ball, the
    // model read the card and guessed the ball anyway. Transcribing big
    // written text is something it does well; deciding which ball a note
    // belongs to is done by rules in the app (src/domain/writtenNotes.js),
    // and only where the pin count already agrees with the card.
    writtenNotes: {
      type: "array",
      description: "Every note written or typed ON TOP OF the screenshot by the bowler -- not part of the app's own scorecard, names, headings, place, date or scores. Usually pin numbers such as \"4\", \"10\" or \"1-3-6\" written beside or below a game. Check every image separately. The same number written on two images is two notes: list each one, with its own imageNumber -- never merge them. Empty if there are none.",
      items: {
        type: "object",
        properties: {
          text: { type: "string", description: "The note exactly as written, e.g. \"1-3-6\"." },
          gameNumber: { type: "integer", nullable: true, description: "The game row the note sits next to or directly below. Null if unclear." },
          bowlerName: { type: "string", nullable: true, description: "Whose screenshot the note is on, as printed on the card, when the card shows more than one bowler." },
          imageNumber: { type: "integer", nullable: true, description: "Which image the note is written on: 1 for the image labelled \"Image 1\", 2 for \"Image 2\", and so on." },
        },
        required: ["text"],
      },
    },
...
const EXTRACTION_PROMPT = `You are reading a bowling scorecard -- a screenshot from a scoring app (LaneTalk and others), a photo of a lane monitor, or a printed sheet. Extract every game and frame shown into the exact JSON shape requested.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const parts = [
      ...
      // Each image labelled, so a written note can say which image it is
      // on -- a team's card is a screenshot per bowler, and the app places
      // a note by the image it was written on.
