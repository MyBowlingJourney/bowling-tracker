...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const detailedSuffix = detailed && !counting
      ? `\n\nTHIS CARD HAS BEEN CHECKED AND DOES SHOW PER-FRAME DETAIL. For this card, an empty frames array is WRONG. Transcribe every frame of every game you can see -- all ten frames per game, each ball's pins, marks for strikes (X) and spares (/). Use each frame's running total printed on the card to check your reading, and make each game's frames add up to its printed totalScore. Where a single pin is genuinely unreadable, give your best reading consistent with the running total rather than dropping the frame. Also give every game its bowlerName exactly as printed on the card, and its printed totalScore.\n\nHOW TO READ THE PIN DECK ABOVE EACH FRAME. Read this carefully -- getting the sides backwards is the most common mistake.\n\nLAYOUT. Each frame has a small triangle of ten pins drawn as seen FROM THE BOWLER, looking down the lane:\n- TOP row, four pins, left to right: 7, 8, 9, 10\n- second row, three pins, left to right: 4, 5, 6\n- third row, two pins, left to right: 2, 3\n- BOTTOM, one pin: 1 (the head pin, nearest the bowler)\nSo the TOP-LEFT pin is ALWAYS 7 and the TOP-RIGHT pin is ALWAYS 10. A right-hander's common single-pin leave is the 10 pin: the TOP-RIGHT dot. Never mirror this.\n\nCOLOURS. Every pin is one of three states:\n- plain GREY: knocked down by the FIRST ball.\n- GREEN filled dot: left standing by the first ball, then knocked down by the SECOND ball.\n- WHITE / hollow ring: still standing after BOTH balls (missed).\n\nHOW TO FILL pinsStanding.\n- Ball 1 of a frame: pinsStanding = every GREEN pin plus every WHITE pin (all that were up after ball 1).\n- Ball 2: pinsStanding = only the WHITE pins. If the frame is a spare (/), ball 2 pinsStanding is empty.\n- A strike (X) has no ball 2 in frames 1-9, and pinsStanding is empty.\n\nCHECK EVERY FRAME AGAINST ITS MARKS -- the numbers printed under the deck win if they disagree with your reading of the dots:\n- The first mark is the first-ball COUNT: "9" means exactly ONE pin was left (10 - 9), so ball 1 pinsStanding has exactly one pin. "7" means three pins. A circled number (like a ringed 7) means the leave was a split.\n- "/" means the second ball knocked down every remaining pin.\n- "-" means the second ball knocked down nothing: ball 2 pinsStanding equals ball 1 pinsStanding (all WHITE).\n- A number in the second position is how many the second ball knocked down, so ball 2 pinsStanding has (first leave - that number) pins.\n- The running total under the frame must match your frames scored in order.\n\nTHE 10TH FRAME can show up to three marks ("X 8 /", "9 / 9", "X X X", "X 7 1") but only ONE drawing of the rack. Use the marks for every ball; use the drawing only for the rack that was NOT a strike (the pins after the one non-strike ball). "X 8 /": ball 1 strike, ball 2 left two pins, ball 3 spare. "9 / 9": ball 1 left one pin, spare, then a fill ball of 9 (one pin left standing). Each ball of the 10th is its own entry in balls.`
      : "";
